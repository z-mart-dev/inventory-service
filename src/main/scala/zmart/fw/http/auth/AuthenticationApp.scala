  ...
  def jwtDecode(header: String): Option[JwtClaim] = {
    if (header.startsWith("Bearer")) {
      for {
        ...
        claim <- Jwt.decode(token, AuthHelper.SECRET_KEY, Seq(JwtAlgorithm.HS512)).toOption
      } yield claim
    } else {
      Option.empty
    }
  ...
  def login: Http[Any, HttpError.Unauthorized, Request, UResponse] = Http
    .collectM[Request] { case req @ Method.POST -> Root / "login" =>
      for {
        ...
        authenticatedUser <- AuthHelper.validateLogin(loginRequest)
        jwtClaim          <- ZIO.effect(AuthHelper.jwtEncode(authenticatedUser.username))
